...
export async function shraniKategorijo(formData) {
  ...
  const podatki = {
    naziv,
    slug: slugify(formData.get("slug")?.trim() || naziv),
...
export async function shraniNovico(formData) {
  ...
  const podatki = {
    naslov,
    slug: slugify(formData.get("slug")?.trim() || naslov),
...
// ---------------- VSEBINSKI BLOKI ----------------
export async function shraniVsebino(formData) {
  await zahtevajAdmina();
  const kljuc = formData.get("kljuc");
  await supabaseAdmin()
    .from("vsebine")
    .update({
      naslov: formData.get("naslov")?.trim() || null,
      besedilo: formData.get("besedilo")?.trim() || null,
      slika_url: formData.get("slika_url")?.trim() || null,
    })
    .eq("kljuc", kljuc);

  revalidatePath("/admin/vsebine");
  revalidatePath("/");
  redirect("/admin/vsebine");
}

// ---------------- UVOZ ARTIKLOV IZ EXCELA ----------------
// Prilepi stolpce iz preglednice. Vrstice locuje nova vrstica,
// stolpce tabulator ali podpicje. Obstojeci artikli se posodobijo po sifri.
export async function uvoziArtikle(prejsnje, formData) {
  await zahtevajAdmina();

  const izdelekId = formData.get("izdelek_id");
  const surovo = (formData.get("podatki") || "").trim();
  if (!surovo) return { stanje: "napaka", sporocilo: "Polje je prazno." };
  ...
  vrstice.forEach((vrstica, i) => {
    const p = vrstica.split(/\t|;/).map((x) => x.trim());
    const [sifra, naziv, dimenzija, ean, pakiranje, cena, zaloga, teza] = p;

    if (i === 0 && /sifra|šifra/i.test(sifra || "")) return;
    if (!sifra || !naziv) {
      preskoceno += 1;
      return;
    }
    ...
    artikli.push({
      ...
      ean: ean || null,
      pakiranje: stevilo(pakiranje),
      cena_mpc: stevilo(cena),
      zaloga: stevilo(zaloga),
      teza_g: stevilo(teza),
      objavljeno: true,
      v_trgovini: true,
    });
  ...
  if (artikli.length === 0) {
    return { stanje: "napaka", sporocilo: "Ni uporabnih vrstic. Prvi stolpec mora biti šifra, drugi naziv." };
  }

  const { error } = await supabaseAdmin()
    .from("artikli")
    .upsert(artikli, { onConflict: "sifra" });

  if (error) return { stanje: "napaka", sporocilo: error.message };

  revalidatePath(`/admin/izdelki/${izdelekId}`);
  return {
    stanje: "ok",
    sporocilo: `Uvoženih ${artikli.length} artiklov.${preskoceno ? ` Preskočenih vrstic: ${preskoceno}.` : ""}`,
  };
}

// ---------------- GALERIJA IZDELKA ----------------
export async function dodajVGalerijo(formData) {
  await zahtevajAdmina();
  const izdelekId = formData.get("izdelek_id");
  const url = formData.get("slika_url")?.trim();
  if (!url) return;

  const sb = supabaseAdmin();
  const { data } = await sb.from("izdelki").select("galerija").eq("id", izdelekId).single();
  const galerija = Array.isArray(data?.galerija) ? data.galerija : [];

  if (!galerija.includes(url)) {
    await sb.from("izdelki").update({ galerija: [...galerija, url] }).eq("id", izdelekId);
  }
  revalidatePath(`/admin/izdelki/${izdelekId}`);
}

export async function odstraniIzGalerije(formData) {
  await zahtevajAdmina();
  const izdelekId = formData.get("izdelek_id");
  ...
  await sb
    .from("izdelki")
    .update({ galerija: galerija.filter((g) => g !== url) })
    .eq("id", izdelekId);
...
// ---------------- DELOVNA MESTA ----------------
export async function shraniDelovnoMesto(formData) {
  await zahtevajAdmina();
  const sb = supabaseAdmin();
  ...
  const podatki = {
    naziv,
    slug: slugify(formData.get("slug")?.trim() || naziv),
    lokacija: formData.get("lokacija")?.trim() || null,
    vrsta: formData.get("vrsta")?.trim() || null,
    kratek_opis: formData.get("kratek_opis")?.trim() || null,
    opis: formData.get("opis")?.trim() || null,
    naloge: seznam(formData.get("naloge")),
    pricakujemo: seznam(formData.get("pricakujemo")),
    ponujamo: seznam(formData.get("ponujamo")),
    slika_url: formData.get("slika_url")?.trim() || null,
    rok_prijave: formData.get("rok_prijave") || null,
    vrstni_red: Number(formData.get("vrstni_red") || 0),
    aktivno: formData.get("aktivno") === "on",
    seo_naslov: formData.get("seo_naslov")?.trim() || null,
    seo_opis: formData.get("seo_opis")?.trim() || null,
  };

  if (id) await sb.from("delovna_mesta").update(podatki).eq("id", id);
  else await sb.from("delovna_mesta").insert(podatki);

  revalidatePath("/admin/delovna-mesta");
  revalidatePath("/zaposlitev");
  redirect("/admin/delovna-mesta");
}

export async function izbrisiDelovnoMesto(formData) {
  await zahtevajAdmina();
  await supabaseAdmin().from("delovna_mesta").delete().eq("id", formData.get("id"));
  revalidatePath("/admin/delovna-mesta");
  revalidatePath("/zaposlitev");
}
